...
To bootstrap continuous delivery in the local development environment, ArgoCD is deployed directly inside the Kind cluster in its own namespace (\texttt{argocd}). Because Gitea runs in a separate Docker container on the host machine, pods inside the cluster resolve Gitea via the native host gateway DNS entry \texttt{host.docker.internal:3000}.
...
\begin{enumerate}
    ...
    \item \textbf{Gitea Code Mirroring}: Creates the \texttt{cd} repository inside the Gitea organization if not present, and pushes the local configuration directory state as the GitOps source of truth.
    \item \textbf{Application Mapping}: Configures the root ArgoCD Application to monitor the Gitea repo's \texttt{helm/} path and automatically synchronize changes into the cluster.
\end{enumerate}

To route traffic to the ArgoCD web console, we define a Kubernetes Ingress resource mapping the domain \texttt{argocd.erp-uno.local} to the \texttt{argocd-server} service, utilizing the cluster's pre-configured Traefik Ingress controller:
...
If the UI is not accessible directly or if editing \texttt{/etc/hosts} is restricted, port forwarding to the ArgoCD Server service serves as a fallback:
\begin{verbatim}
