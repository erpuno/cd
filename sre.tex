...
\lstdefinestyle{shell}{
  ...
  commentstyle=\color{footergray}\itshape,
}
\lstset{style=shell}
...
\titleformat{\section}{\color{primary}\Huge\bfseries}{\thesection.}{1em}{}
\titlespacing*{\section}{0pt}{32pt}{14pt}
\titleformat{\subsection}{\color{primary}\LARGE\bfseries}{\thesubsection.}{1em}{}
\titlespacing*{\subsection}{0pt}{22pt}{10pt}
\titleformat{\subsubsection}{\color{primary}\large\bfseries}{\thesubsubsection.}{1em}{}
...
\hypersetup{colorlinks=true,linkcolor=primary,urlcolor=accent,
  pdftitle={ERP/1 SRE Handbook},pdfauthor={Zen Crypted}}
...
\color{white}\thispagestyle{empty}\vspace*{3cm}\centering
  ...
  {\LARGE \textbf{Архітектура. Компоненти. Надійність.\\Helm GitOps для Kubernetes} \par}
  \vspace{1.5cm}
  \textcolor{white!70}{\rule{8cm}{0.4pt}}
  \vspace{1.5cm}
  \begin{tabular}{rl}
    \textbf{Версія системи:}  & 2024.6.15 \\[4pt]
    \textbf{Кластер:}         & desktop-control-plane (arm64) \\[4pt]
    \textbf{Namespace'и:}     & 6 просторів імен ERP \\[4pt]
    \textbf{Компоненти:}      & 20 мікросервісів \\[4pt]
...
\texttt{erp-infra}
$\rightarrow$ \texttt{erp-telemetry}
$\rightarrow$ \texttt{erp-security}
$\rightarrow$ \texttt{erp-ai}
$\rightarrow$ \texttt{erp-services}
$\rightarrow$ \texttt{erp-apps}\\[4pt]
...
\texttt{ns-dns}          & erp-infra  & Deployment  & 8080 & 100m/500m & deployment $\cdot$ service $\cdot$ values \\
\rowcolor{rowalt}\texttt{docker-registry} & erp-infra & Deployment & 5000 & 50m/200m & deployment $\cdot$ service $\cdot$ pvc(10Gi) $\cdot$ values \\
\midrule
\multicolumn{6}{l}{\textcolor{successgreen}{\bfseries\small erp-telemetry --- Observability рівень}}\\
\texttt{prometheus}      & erp-telemetry & StatefulSet & 9090 & 100m/500m & deployment $\cdot$ service\textsuperscript{HL} $\cdot$ pvc(10Gi) $\cdot$ values \\
\rowcolor{rowalt}\texttt{grafana}  & erp-telemetry & Deployment & 3000 & 50m/200m & deployment $\cdot$ service $\cdot$ pvc(10Gi) $\cdot$ values \\
\texttt{loki}            & erp-telemetry & Deployment  & 8403 & 100m/500m & deployment $\cdot$ service $\cdot$ values \\
\rowcolor{rowalt}\texttt{otel-collector} & erp-telemetry & Deployment & 8404 & 100m/500m & deployment $\cdot$ service $\cdot$ values \\
\midrule
\multicolumn{6}{l}{\textcolor{secpurple}{\bfseries\small erp-security --- Безпековий рівень}}\\
\texttt{ca-pki}          & erp-security & Deployment & 8201 & 100m/500m & deployment $\cdot$ service $\cdot$ values \\
\midrule
\multicolumn{6}{l}{\textcolor{tieryellow}{\bfseries\small erp-ai --- AI рівень}}\\
\rowcolor{rowalt}\texttt{ai-generation} & erp-ai & Deployment & 8601 & 100m/500m & deployment $\cdot$ service $\cdot$ values \\
\midrule
\multicolumn{6}{l}{\textcolor{tieryellow}{\bfseries\small erp-services --- Сервісний рівень}}\\
\texttt{rest-bpe}        & erp-services & Deployment & 8303 & 100m/500m & deployment $\cdot$ service $\cdot$ values \\
\rowcolor{rowalt}\texttt{bpe-engine}   & erp-services & Deployment & 8302 & 100m/500m & deployment $\cdot$ service $\cdot$ values \\
\texttt{n2o-server}      & erp-services & Deployment & 8511 & 100m/500m & deployment $\cdot$ service $\cdot$ values \\
\rowcolor{rowalt}\texttt{kvs-database} & erp-services & Deployment & 8301 & 100m/500m & deployment $\cdot$ service $\cdot$ values \\
\texttt{chat-messenger}  & erp-services & Deployment & 8507 & 100m/500m & deployment $\cdot$ service $\cdot$ values \\
\midrule
\multicolumn{6}{l}{\textcolor{tieryellow}{\bfseries\small erp-apps --- Бізнес рівень}}\\
\rowcolor{rowalt}\texttt{hl7-health}   & erp-apps & Deployment & 8502 & 100m/500m & deployment $\cdot$ service $\cdot$ hpa(2--6) $\cdot$ values \\
\texttt{lms-education}   & erp-apps & Deployment & 8506 & 100m/500m & deployment $\cdot$ service $\cdot$ hpa(2--6) $\cdot$ values \\
\rowcolor{rowalt}\texttt{wms-warehouse}& erp-apps & Deployment & 8505 & 100m/500m & deployment $\cdot$ service $\cdot$ values \\
\texttt{acc-accounting}  & erp-apps & Deployment & 8504 & 100m/500m & deployment $\cdot$ service $\cdot$ values \\
\rowcolor{rowalt}\texttt{crm-documents}& erp-apps & Deployment & 8503 & 100m/500m & deployment $\cdot$ service $\cdot$ values \\
\texttt{cart-registers}  & erp-apps & Deployment & 8501 & 100m/500m & deployment $\cdot$ service $\cdot$ values \\
\rowcolor{rowalt}\texttt{pm-projects}  & erp-apps & Deployment & 8514 & 100m/500m & deployment $\cdot$ service $\cdot$ values \\
\texttt{itsm-incidents}  & erp-apps & Deployment & 8513 & 100m/500m & deployment $\cdot$ service $\cdot$ values \\
\end{longtable}
}% end footnotesize
